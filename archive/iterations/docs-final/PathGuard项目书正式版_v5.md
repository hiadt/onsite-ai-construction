...
系统将离线建模与在线使用分开。离线流程处理冻结样本、地图分组、模型比较与工件固化；在线流程处理用户输入、固定特征变换、独立分析及评估记录。在线分析不会读取当前路线的最终通过或失败标签来生成排序分数，验证结果在评估之后进入证据管理模块。由此，模型训练可以利用历史结果，现场使用则保持执行前输入的可获得性。

![fig_system_detail](assets_v5/fig_system_detail.png)

图1 系统模块与数据接口 实线为当前调用或数据传递 虚线为需审核的新版本迭代

四条分析通道具有不同的输入和输出。学习通道将55维特征映射为历史失败倾向；规则通道将16项特征映射为六组相对工程应力；空间通道在车辆与边界条件满足时计算局部余量；执行证据通道从已有遥测重建横向和航向偏差。最后一条需要实际执行记录，不能作为新路线执行前已经知道的输入。当前产品将各通道结果并列组织为工程任务，而不是将它们未经标定相加为一个综合概率。

表4 核心模块的输入输出与失效处理

| 模块 | 输入与输出 | 条件不满足时的处理 |
|---|---|---|
| route_input | 路线NPZ和车辆配置 → 特征、完整几何、展示几何 | 数组异常停止解析；缺边界保留路线展示 |
| inference | 有序55维特征和模型工件 → 失败倾向分值 | 契约或版本不匹配停止模型推断 |
| rule_baseline | 16项工程特征和固定参考分布 → 六组应力 | 缺字段或非有限数值报错，不自动补零 |
| geometry_integrity与空间计算 | 全分辨率路线、左右边界和尺寸 → 条件性余量 | 筛查未通过则不给出有效边界计算结论 |
| tracking_evidence | 配对遥测与参考路线 → 误差曲线与事件 | 配对不一致的记录不作为确认案例 |
| feedback_store | 评估快照、验证结果与条件 → 可追溯记录 | 同条件冲突保留多条证据并阻止单一标签准入 |

这里的模块划分提供了明确的替换边界。模型升级只需继续满足特征契约和概率输出接口；几何方法升级需要另行维护车辆和环境语义；执行记录增加不会改变已保存评估快照。各模块不共享一个模糊的“可信度百分比”，而是保留可以核对的输入条件和版本标识。
...
对第i条路线，将按里程排序的点列记为轨迹对象。每个采样点包含里程、平面位置、高程、曲率、计划速度及左右距离；转向与横摆参考通道按可用字段解析。车辆配置和最终验证标签属于独立对象，不能因为与路线同时存储就全部进入模型。现有特征变换记为Φ，输出长度固定为55的实数向量。


$$
\mathcal{T}_i=\{s_j,p_j,z_j,\kappa_j,v_j,c^L_j,c^R_j\}_{j=1}^{N_i},\quad x_i=\Phi(\mathcal{T}_i)\in\mathbb{R}^{55}
$$

式（1）

输入统一采用米、秒和弧度；里程必须严格递增，所需数组必须等长且为有限数值。位置与边界还需确认坐标系一致。数组中即使同时出现滑移率、轮胎利用率或实际跟踪误差等执行后结果，提取器也不能将它们混入训练特征。这一约束对应“部署时是否能够获得该输入”，是避免结果泄漏的第一道边界。

表6 55维特征的计算分解 本表按公式含义划分 不等同于消融脚本的字符串分组

| 特征族 | 维数 | 统计量与工程含义 |
|---|---|---|
| 路线与采样 | 5 | 长度、点数、间距均值、中位数和P95 |
| 曲率与变化 | 10 | 有符号均值、绝对值统计、沿里程变化统计 |
| 坡度与变化 | 7 | 高程一阶与二阶空间变化统计 |
| 计划速度 | 5 | 均值、最大值及三个分位数 |
| 速度曲率耦合 | 6 | 速度乘曲率、速度平方乘曲率及相关系数 |
| 转向幅值与变化 | 8 | 控制通道绝对幅值及沿里程变化 |
| 横摆参考与变化 | 6 | 横摆角速度绝对值及空间变化统计 |
| 静态边界距离 | 3 | 左侧、右侧及两侧最小距离 |
| 局部窗口 | 4 | 10米窗口内曲率、转向变化及左右距离统计 |
| 转向输入维度 | 1 | 原控制数组通道数，不直接等价于车辆轴数 |

### 4.6 非均匀采样下的导数与局部窗口

不同路线的采样间距可能不同，因此代码按实际里程调用numpy.gradient，而不是简单把相邻数值相减。令前后间距分别为h₋和h₊，内点的一阶导数可表示为下式。边界在至少三个点时使用二阶单边近似；两个点时使用一阶近似。已有曲率导数数组优先读取，缺失时才由曲率和里程推导。


$$
D_s f_j=-\frac{h_+ f_{j-1}}{h_-(h_-+h_+)}+\frac{(h_+-h_-)f_j}{h_-h_+}+\frac{h_- f_{j+1}}{h_+(h_-+h_+)}
$$

式（2）

转向数组若有多个通道，先取每个点各通道绝对值的最大值，再计算这一幅值包络沿里程的变化。因此，转向变化特征不是每个轮轴转角导数的最大值，两种运算次序通常不等价。横摆角速度优先使用已有参考通道；其次使用每米横摆量乘速度；再次回退为曲率乘速度。每一种回退均写入来源说明。


$$
u_j=\max_k|\delta_{jk}|,\quad g_j=D_s z_j,\quad \widehat{\omega}_j=v_j\kappa_j,\quad a^{\rm proxy}_{y,j}=v_j^2|\kappa_j|
$$

式（3）

速度平方乘曲率是平面运动学中的横向加速度代理量。它能表达高速急弯比低速缓弯更值得关注，但不包含轮胎附着、载荷转移或车身侧倾方程，不能把该代理量直接称为车辆实测侧向力。

10米窗口采用向后看的里程范围，并通过前缀和计算窗口内采样点均值。它强调某段路线持续出现的应力，而不只关心单点峰值。设起点索引a(j)为第一个满足里程不小于sⱼ减10米的点，则：


$$
a(j)=\min\{k:s_k\ge s_j-10\},\quad P_j=\sum_{k=1}^{j}f_k,\quad \bar f_j^{(10)}=\frac{P_j-P_{a(j)-1}}{j-a(j)+1}
$$

式（4）

当前窗口统计是采样点的算术平均，尚未改为距离加权积分。因此采样密度本身也是输入质量的一部分。前缀和为线性复杂度，窗口起点的向量化二分查询约为N对数N量级；分位数统计和梯度运算由数值库完成。展示点可抽稀以减少绘图开销，空间计算保留完整分辨率，二者不能混用。
...
$$
x_i\in\mathbb{R}^{55},\quad y_i\in\{0,1\},\quad r_i=\widehat P(y_i=0\mid x_i)
$$
...
选择该模型不是因为模型名称更复杂，而是因为开发集比较提供了依据：地图分组五折中，直方图梯度提升树失败类别AP指标均值为0.6605，ExtraTrees为0.6349，逻辑回归为0.6157。该比较支持其作为现阶段冻结基线，同时折间波动提示需要继续扩展独立地图与真实任务。项目的技术价值也因此不押注单一算法，而是保留可替换模型接口和一致的评价协议。
...
冻结工件采用HistGradientBoostingClassifier。训练标签y为通过状态，1表示通过，0表示失败；内部二分类得分F对应通过类别的对数几率。产品使用通过概率的补数作为失败倾向。该输出用于历史模式下的排序，不自动获得概率校准或现场事故概率的含义。[4]


$$
F_T(x)=F_0+\eta\sum_{t=1}^{T}h_t(x),\quad p_{\rm pass}(x)=\frac{1}{1+e^{-F_T(x)}},\quad r(x)=1-p_{\rm pass}(x)
$$

式（6）

每轮树学习当前预测尚未解释的残差。二元对数损失惩罚自信但错误的判断；二阶近似使用梯度和曲率信息计算叶节点更新量。下面给出与当前二分类提升过程一致的常规数学表达，忽略实现中的常数倍缩放。


$$
\ell(y,F)=-y\log p-(1-y)\log(1-p),\quad g=p-y,\quad h=p(1-p)
$$
...
$$
w_A=-\frac{\sum_{i\in A}g_i}{\sum_{i\in A}h_i+\lambda},\quad F_t(x)=F_{t-1}(x)+\eta w_{A_t(x)}
$$

式（8）

直方图方法将连续特征映射为有限个分箱，累积各箱的梯度与Hessian，再比较候选分裂的损失下降。分箱降低了逐个连续值尝试切分的成本；叶节点限制与L2正则共同控制小数据下的拟合复杂度。当前工件最多255个分箱、每棵树最多15个叶节点，学习率0.04，共250轮，最小叶样本数20，L2系数1.0，随机种子20260922。以上来自实际加载工件的get_params，而不是空间扩展实验的参数。

为说明分裂准则，设候选节点分为左右子节点，G和H分别为其中梯度与Hessian之和。常规二阶损失下降与下式成正比；实现另受最小叶样本数等约束。这里没有人为加入XGBoost特有的额外分裂惩罚参数。


$$
\Delta\mathcal{L}\propto\frac{G_L^2}{H_L+\lambda}+\frac{G_R^2}{H_R+\lambda}-\frac{(G_L+G_R)^2}{H_L+H_R+\lambda}
$$

式（9）

这种结构可学习“曲率并不极端，但速度和转向变化共同偏高”之类非线性交互。它仍然依赖历史标签覆盖；如果训练数据中没有相应车辆尺寸和工况变化，不能靠树模型名称推导其具备未学习到的参数泛化能力。当前车长、车宽不是这55维输入中的独立变量，其调整进入几何分析通道。

### 5.7 模型选择 评估工件与部署工件

训练脚本首先按地图构建274条开发样本和66条历史测试样本。划分函数在300个分组随机候选中，根据样本比例和类别比例接近程度选择划分；选择标准不包含模型得分，但使用了标签比例，因此应称为按地图隔离并兼顾类别平衡的历史留出。开发集内再做五折GroupKFold，比较逻辑回归、ExtraTrees和直方图梯度提升树。[3]

![fig_training_detail](assets_v5/fig_training_detail.png)

图2 地图分组训练与产品发布的两条路径 历史留出分数不由全量演示工件重算

逻辑回归基线使用标准化、C等于0.5和类别平衡；ExtraTrees使用500棵树、最小叶样本2和平方根特征采样；HGB采用前节参数。以开发集五折失败AP均值优先、ROC均值次之选择模型族。这里的比较证明的是当前数据和协议下的选择依据，不是对所有模型家族的穷尽最优搜索。

历史留出评估路径将开发地图进一步分为拟合集和校准集：先拟合基础树模型，再在开发校准集拟合sigmoid校准器，然后计算66条留出结果。阈值曾在同一校准集上遍历0.05到0.95共181个值，以失败类别F2最大为目标。由于校准与阈值选择共用该子集，阈值表现仍可能乐观；它不构成新场景安全阈值。

产品部署路径则在全部340条冻结标签上重新拟合所选HGB，并保存到joblib。实际部署工件是未包裹校准器的HGB，不能将历史校准模型的概率性质或0.45阈值直接当成部署工件已验证的概率准则。界面上的历史留出实验、全量工件回放和新任务评分应保持三种身份。这个区分使演示可使用全部已知数据，同时不把拟合过的记录伪装成新的独立测试。

### 5.8 固定经验分布规则与工程解释

对每个规则特征保留开发集274条样本的有序参考值。设某值x左侧严格小于它的参考数为n₍₍，等于它的参考数为n₌，中秩经验分位数如下。使用中秩可避免大量并列值全部落在同一侧的极端处理。


$$
\widehat F_f^{\rm mid}(x)=\frac{\#\{v<x\}+\tfrac12\#\{v=x\}}{M},\quad M=274
$$

式（10）

大值表示更值得关注的指标直接取该分位数，小值表示更值得关注的净空类指标取其补数。先在每组内部求均值，再对六组等权平均。各组特征数依次为3、2、3、2、1和5，因此它并不是16个特征的简单等权平均。


$$
S_g=\frac{1}{|J_g|}\sum_{f\in J_g}s_f(x_f),\quad s_f(x)=\widehat F_f^{\rm mid}(x)\ \mathrm{or}\ 1-\widehat F_f^{\rm mid}(x),\quad R=\frac16\sum_{g=1}^{6}S_g
$$

式（11）

例如参考值为1、2、2、4时，查询2得到分位数0.5；高值敏感与低值敏感方向均为0.5。查询4得到0.875，高值敏感指标被重点关注，而低值敏感指标仅为0.125。这是说明算法的四点示例，实际产品仍使用冻结的274条分布。上传另一批候选不会重算参考分布，因此同一输入的规则得分可以跨批次比较。

工程原因由这些可解释的指标和参考分布产生，回答“哪些输入值得核对”。它不是SHAP贡献，也不是把学习模型的总分精确拆成六个可加部分。模型与规则的分歧能形成复核线索，但不能仅凭分歧判断哪一个一定正确。

### 5.9 验证预算与独立事项的队列策略

设本次候选集合为C，需预算外单列处理的集合为M，用户普通验证预算为K。模型可用时按失败倾向降序排列，否则使用规则预览分值。普通队列从C排除M后取前K条，最终结果为两类事项的并集。预算外事项优先显示，同分保留输入顺序。


$$
Q=M\cup\operatorname{TopK}(C\setminus M;r),\quad |Q|=|M|+\min(K,|C\setminus M|)
$$

式（12）

![fig_queue_detail](assets_v5/fig_queue_detail.png)

图3 规则解释与验证队列的独立决策链 当前combined_risk字段沿用模型分值

例如本轮共有12条候选、普通预算3条，其中2条符合预算外条件，最终可以显示5条。多出的2条不表示系统擅自提高普通复核率，而是不同工作项的合并呈现。选择前10%时，以全部候选数量向上取整得到普通预算，再排除预算外集合选取。假设尺寸引起的空间试算不会自动触发真实任务的越界事项。

算法1 在线评估与队列生成

① 解析输入并保存路线指纹；提取55维特征并按契约顺序排列。
② 校验模型版本、Schema哈希和输入维数；通过时输出模型分值，否则标明规则预览。
③ 用冻结参考分布计算六组工程应力，保存分值与原因；不与模型强行相加。
④ 单独计算有条件的空间结果，并由具体业务条件形成预算外事项集合。
⑤ 从其余候选中取普通预算，合并两类工作项，保存输入、模型与条件版本。
...
$$
e_y=(p_{act}-p_{ref})\cdot[-\sin\psi_{ref},\cos\psi_{ref}]
$$
...
$$
e_\psi=\operatorname{wrap}(\psi_{act}-\psi_{ref})
$$
...
$$
n_c=e_y+x_c\sin e_\psi+y_c\cos e_\psi
$$
...
当前几何模型将车身视为平面刚性矩形。令长度为L，宽度为W，车辆参考点到后端的纵向距离为a，则车身四角相对参考点的局部坐标如下。a等于L的一半时参考点位于几何中心；实际应用应采用车辆文件明确给出的参考点。


$$
\mathcal C=\{(-a,-W/2),(-a,W/2),(L-a,W/2),(L-a,-W/2)\},\quad 0\le a\le L
$$
...
$$
q_{ij}=p_i+R(\psi_i)c_j,\quad R(\psi)=\begin{bmatrix}\cos\psi&-\sin\psi\\\sin\psi&\cos\psi\end{bmatrix}
$$

式（17）

逐点变换得到姿态序列上的外廓，外廓在时间或里程上的并集描述扫掠范围。当前方法不解轮胎侧偏、车轴转向运动学或铰接动力学。五轴和六轴示意帮助识别车辆结构，但只要使用同一刚性外廓与相同姿态，轴数本身不会额外改变这一几何变换。

![fig_geometry_detail](assets_v5/fig_geometry_detail.png)

图4 参考点 车身坐标与边界截面的对应关系 图中尺寸和余量仅为机制示意

### 6.7 从边界点列到条件性余量

边界筛查首先检验点列形状、有限数值、里程递增以及中心点不重复。随后检查左右边界到中心点的欧氏距离能否复现文件给出的左右距离。距离一致性只能支持字段语义对应，不能证明整个道路几何正确。


$$
E_d=\max_i\left\{\left|\|b_i^L-p_i\|-c_i^L\right|,\left|\|b_i^R-p_i\|-c_i^R\right|\right\}\le10^{-5}\ \mathrm m
$$

式（18）

再以路线切向量的左法向为基准检查左右方向、左右次序和点列连续性。筛查要求方向与次序正确比例分别至少99%；边界相邻步长除以中心线相邻步长的单侧P99.9最大值不超过3，最大比值不超过5。这些是当前实现的数据筛查参数，不是行业安全距离标准。后续计算入口还要求左右方向正确比例严格大于99%，且不存在左右颠倒点，属于更严格的二次条件检查。

实际余量计算在矩形四条边上各取四个周界点，共16点。局部纵坐标为cⱼₓ的车身点，以sᵢ加cⱼₓ近似映射到道路截面里程，再对中心线和边界作线性插值。该映射没有求解曲线最近点投影，因此需明确称为局部截面近似。


$$
\widetilde s_{ij}=\operatorname{clip}(s_i+c_{jx},s_0,s_{N-1}),\quad d^L=\|b^L(\widetilde s)-p(\widetilde s)\|,\quad u^L=\frac{b^L(\widetilde s)-p(\widetilde s)}{d^L}
$$
...
$$
m^L_{ij}=d^L-u^L\cdot(q_{ij}-p(\widetilde s_{ij})),\quad m_i=\min_j\{m^L_{ij},m^R_{ij}\}
$$

式（20）

右侧余量按同样方式计算。负值表示某个被采样的车身点超出了该局部截面限制；非负但小于复核余量参数时，提示余量偏小。默认复核参数为0.5米，它用于当前展示策略，不代表所有工程车辆统一合格标准。当前输出保留最小值、路线里程与左右侧别，形成“哪里接近、靠近哪侧、还剩多少”的解释；尚未持久化最危险周界点对应的车头或车尾分类。

低余量区间长度使用相邻端点并集规则：只要相邻两个采样点中至少一个低于阈值，该段里程计入统计。因此它是离散近似区间长度，并非精确求解越界交点的连续长度。


$$
\widehat L_\tau=\sum_{i=1}^{N-1}(s_{i+1}-s_i)\,\mathbf1\{m_i<\tau\ \lor\ m_{i+1}<\tau\}
$$
...
① 校验车辆长度、宽度和参考点；确认参数是实配还是假设试算。
② 完成边界距离语义、方向、次序和连续性筛查；失败则输出具体缺失原因。
③ 读取全分辨率姿态和边界，对16个周界点做刚体变换及截面里程映射。
④ 插值得到局部中心和两侧限制，计算投影余量，汇总最小值、低余量区间和位置。
⑤ 若为假设配置，结果保留为试算；只有满足正式输入条件的异常才参与任务事项判断。

该方法把可调车长和车宽连接到了具体的空间结果，形成可操作的参数解释。有限周界采样、端点截断和局部截面映射仍可能遗漏复杂道路形状中的问题，当前不将其等同于多边形连续碰撞检测。下一阶段若要升级，应分别验证最近点投影、边界自交处理和连续扫掠，而不能只提高图形刷新帧率。

### 6.8 跟踪偏差与空间占用的敏感性

对局部车身点的法向坐标公式求导，可得到横向偏差与航向偏差的不同作用。横向误差对每个点产生相同平移；航向误差的影响则随纵向力臂变化。下式为现有几何公式的解析解释，不是新增的学习模型。


$$
\frac{\partial n_c}{\partial e_y}=1,\quad \frac{\partial n_c}{\partial e_\psi}=x_c\cos e_\psi-y_c\sin e_\psi,\quad n_c\approx e_y+y_c+x_c e_\psi
$$

式（22）

小角度近似要求角度采用弧度。参考点居中的18.8米矩形，其前后端力臂约9.4米；5度方向偏差产生的纵向力臂投影约0.819米，而车宽的余弦项还会小幅变化。这解释了为什么长车在相同中心点误差下仍可能有不同外廓占用。该导数揭示几何敏感性，并不预测车辆未来一定出现多少航向偏差。

实际执行分析先以记录的里程进度插值参考位置和航向，再将位置差投影到参考法向。航向差环绕到主值区间，避免跨越正负π时出现不真实尖峰。重算结果与原始记录比较后，再用于解释案例。空间余量、平台碰撞字段和文本净空可能来自不同定义，发生分歧时应保留原始来源和计算条件，而不是选择更有利的一项替代其他记录。
...
一次验证的可比性由路线、环境版本、路线版本和车辆配置共同限定。代码先将车辆配置按字段名排序并序列化，计算配置哈希，再对组合条件计算条件键。哈希用于识别输入是否一致，不表示内容已经由第三方认证。


$$
h_v=\operatorname{SHA256}(\operatorname{CanonicalJSON}(v)),\quad k=\operatorname{SHA256}(\operatorname{JSON}(h_r,e_{ver},r_{ver},h_v))
$$

式（23）

![fig_evidence_detail](assets_v5/fig_evidence_detail.png)

图6 验证记录的准入条件与冲突分支 虚线表示后续人工组织的新版本流程

验证记录保存到本地SQLite，包括评估分值、模型版本、当前配置、验证方式、结果、严重程度和证据引用。用户可以提交仿真、平台或现场结果；系统先检查条件一致性，再识别可比较记录中的通过与失败冲突。实际数据记录采用追加方式，训练审核状态允许显式修改，不能把状态修改描述为删除或覆盖原始证据。

当前训练候选准入代码要求结果明确为通过或失败、状态已核对、条件与评估一致，且相同条件键下不存在相反结果。证据完整度另行展示：已核对且有报告引用，与已核对但证据待补具有不同标识。目前“附件一定存在且经验证”的条件没有成为该准入函数的硬约束，正式外部试点应将人工核查证据可访问性纳入发布流程。

“进入训练候选”是治理状态，不会立即改变模型参数。新增数据仍需经去重、标签审核、任务与路线家族分组、离线比较和版本发布。真实工程结果也不应无条件覆盖仿真：两者若载荷、地面附着、车辆配置或环境版本不同，应形成条件不同的记录；只有同条件冲突才应阻止单一标签生成。
...
| 方法 | ROC指标 | AP指标 | 前10位失败数 | 前20位失败数 | 前34位失败数 |
| --- | --- | --- | --- | --- | --- |
...
表7使用同一历史留出集合，共66条样本、5张地图、39条失败。学习模型前20位包含17条失败，相比随机选择的期望11.82条多约5.18条，相比最小余量排序多7条，展示了多个执行前特征用于历史失败筛查的价值。在前10位时学习模型与规则均为8条；在更大的前34位预算下，规则为27条、模型为24条，说明算法优劣随评价目标和预算变化。
...
用z等于1减y表示失败标签，分值越大越值得优先验证。ROC-AUC可解释为随机抽到的一条失败路线，其分值高于随机通过路线的概率，并对同分计半。平均精确率AP使用召回增量加权各阈值精确率，和对PR折线使用梯形积分不同。当前代码调用average_precision_score，历史列名pr_auc_failure实际对应AP，本文数值按该定义解释。[9]


$$
\mathrm{AUC}=\frac{1}{n_1n_0}\sum_{i:z_i=1}\sum_{j:z_j=0}\left[\mathbf1(r_i>r_j)+\tfrac12\mathbf1(r_i=r_j)\right]
$$
...
$$
\mathrm{AP}=\sum_t(\mathrm{Recall}_t-\mathrm{Recall}_{t-1})\,\mathrm{Precision}_t
$$

式（25）

Top-K精确率回答“选中的路线中有多少确实失败”；失败捕获率回答“全部失败中抓住了多少”。二者分母不同，不能只展示较大的一个而混淆含义。随机抽取K条的预期失败数可以解析计算，不需要编造一次随机实验作为对照。


$$
\mathrm{P@K}=\frac{\sum_{i\in Q_K}z_i}{K},\quad \mathrm{Capture@K}=\frac{\sum_{i\in Q_K}z_i}{F},\quad E[F_{random}]=\frac{KF}{N}
$$

式（26）

66条历史留出中有39条失败。模型Top-10捕获8条，对应精确率80.0%和失败捕获率20.5%，随机预期5.91条；Top-20捕获17条，对应精确率85.0%和捕获率43.6%，随机预期11.82条。高精确率支持优先核查的价值，但不表示只看Top-10就能覆盖大多数失败。

### 8.7 并列分值 分组复核与实验解释

固定预算切在同分组内部时，仅按输入顺序打破并列会使结果受到文件顺序影响。因此历史方法对照采用样本编号次序保证可复现；候选批次代理审计另采用并列组均匀抽取的期望捕获。若阈值以上有n₊条、失败数F₊，同分组有n₌条、失败数F₌，则：


$$
E[F_K]=F_++\frac{K-n_+}{n_=}\,F_=
$$

式（27）

不同口径必须在表头或说明中保留。在线工作队列的稳定排序服务于操作连续性，审计时的期望处理服务于评价公平性，二者不必伪装成完全相同的算法。对于同地图、多尺寸派生路线，分组单位需要覆盖共同来源；如果把同一路线变体分到训练和测试两边，模型可能只记住路线特征而不是真正掌握尺寸泛化。

地图分组复采样可以观察地图组成变化对模型与规则差值的影响：每次有放回抽取地图，再拼接该地图所有样本计算指标差值。当前复核脚本使用2000次地图级抽样，95%区间为差值分布的2.5%与97.5%分位数。这比逐行抽样更尊重同地图相关性，但仅五张留出地图使区间解释仍需谨慎；它不是额外增加2000个独立试验场景。

特征消融在开发地图中删除一组特征并重新训练，回答“在当前协议下去掉该组后排序如何变化”。置换重要性则固定模型，在留出数据上打乱某列并观察AP下降，回答模型对该列关联的依赖。二者都不是因果证明，也不是用户某一条路线的局部解释。当前产品原因标签由规则产生，应与这些离线分析分开呈现。
...
| 技术或策略 | 当前状态 | 用于回答的问题 |
|---|---|---|
| HGB历史失败分类 | 正式工件 | 哪些候选与历史失败模式更接近 |
| 固定中秩经验分位数规则 | 在线独立通道 | 哪些输入相对开发样本更值得关注 |
| 刚体变换和局部截面余量 | 条件性在线计算 | 给定尺寸和边界时哪里空间余量偏小 |
| 遥测误差投影重算 | 已核对案例展示 | 实际执行在哪里偏离参考轨迹 |
| 地图分组验证与消融 | 离线实验 | 跨地图表现与特征组作用如何 |
| sigmoid概率校准 | 历史评估路径 | 历史评估分值如何从基础模型映射 |
| 经验时序响应预测 | 研究复验 | 特定支持范围内能否预测后续偏差 |
| 空间增强学习与模型规则混合 | 研究对照 | 新特征或组合是否提供增量收益 |

当前产品没有部署深度强化学习、模型预测控制、图神经网络、完整数字孪生或SHAP在线归因。项目的学术表达应集中在已有的方法组合、评价协议和证据治理：明确输入时点，解释各分析通道的物理或统计含义，区分历史拟合与独立评估，并把输出连接到可执行的验证任务。这些可复现的细节构成现阶段技术贡献的支撑。
...
[9] scikit-learn developers. average_precision_score. Version 1.0.2 documentation. https://scikit-learn.org/1.0/modules/generated/sklearn.metrics.average_precision_score.html （访问日期2026-09-24）。
...
| 本版统一口径 | docs/final/PathGuard结果口径_v5.md |
...
下表给出实现入口。公式描述的是当前代码或其解析解释，实验性方法已在第八章单独标注。对应源文件的SHA256和冻结工件参数保存在本版technical_v5目录。

| 计算链路 | 实现入口 | 关键复核点 |
|---|---|---|
| 固定特征变换 | extract_gate2_pre_execution_features_shanyu000.py | gradient、rolling_means、extract_features |
| 工件校验与推断 | demo/inference.py | load_gate3_model、predict_failure_risk |
| 经验分位数规则 | demo/rule_baseline.py | _percentile_stress、compute_geometry_rule_risk |
| 队列策略 | demo/demo_logic.py | select_validation_queue、requires_geometry_check |
| 空间变换与余量 | demo/route_input.py | compute_rigid_body_sweep、compute_boundary_rule |
| 边界准入 | demo/geometry_integrity.py | screen_boundary_integrity |
| 验证条件与审核 | demo/feedback_store.py | make_condition_key、set_training_review |
| 历史训练过程 | technical_v5/source_snapshots/run_gate3_original.py | 地图划分、校准路径、全量发布工件 |
| 指标与同分处理 | scripts/audit_heldout_methods_v3.py | 历史方法比较和确定性同分次序 |

### B.2 复现时需要固定的对象

复现一次产品输出至少需要路线文件、特征版本、车辆配置、环境版本、模型工件与规则参考分布。复现一项离线指标还需要逐样本标签、地图划分、分值来源、同分处理方式和预算定义。仅保存一张结果截图不能完整复现计算。

本版补充的是技术描述、架构图与追溯材料，未更换模型权重或修改55维特征契约。数学推导中的示例只用于解释方法，不能增加正式数据规模。原训练脚本快照保留历史上下文和原路径，用于核对训练过程；正式复现实验应在隔离目录配置输入输出，避免直接运行快照覆盖已有工件。
