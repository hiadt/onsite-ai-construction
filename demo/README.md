...
路线详情默认显示原始路线坐标与逐点工程关注位置，可选择最小净空、最大曲率和最大转向变化并放大。页面可调车长、车宽和路线参考点在车身纵向的位置，使用逐点位置与航向角重算米制刚性矩形外廓和四角扫掠轨迹。未确认净空参考定义时不反推道路边界，也不输出碰撞结论。车长、车宽和参考点只改变几何扫掠，不改变当前模型风险或规则风险。

支持直接上传原始NPZ并自动提取55维特征，侧栏提供真实NPZ和车辆配置JSON下载示例。配置可声明vehicle_structure（five_axis/six_axis/unknown）、map_id、length_m、width_m和reference_from_rear_m；后三项仅用于几何显示。内置模型是在全部340条样本上重训的部署模型，验证指标取历史留出预测，内置样本评分不能用来证明泛化性能。
